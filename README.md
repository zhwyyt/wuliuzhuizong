...
Web 管理端默认使用 `调度管理员` 进入管理员视图。

Android App 演示成员账号已内置为手机号登录，例如：
...
- 项目列表、新建项目、按项目查看。
- 人员/设备列表、绑定设备。
- Android App 手机号登录、自动设备绑定、工作区和定位采集模块。
- Web 实时位置展示。
- 所有项目实时分布。
- 单项目筛选。
- 大屏总览页。
...
5. 输入成员手机号和密码，点击“登录”。
6. App 登录成功后会自动为当前成员绑定本机设备，并进入“工作区”。
7. 点击“定位采集”模块，进入采集页。
8. 点击“开始采集”。App 会以前台服务运行，并每 30 秒向后端上报一次真实定位；“立即上报一次”可用于现场验证。
...
- Login account: `13900001002`
- Login password: `123456`
- Bound member: `上海调度员`
- Project: auto-select `上海冷链配送`
- Device: auto-bind from the app, then reuse the saved device credential
...
If the health check opens, the APK can login with the Web-managed phone account, auto-bind the current device, and upload locations to the local backend through Tailscale.
...
The current Android app uses a PDA-style flow: phone login, auto-bind the current device, enter the workspace, and open the location-collection module. It uses AMap location first in the foreground UI and falls back to system location if AMap is unavailable. Background reporting runs as an Android foreground service with a persistent notification and no longer uploads demo coordinates.
...
- Android 端成员账号完全依赖 Web 管理端配置；手机端只负责手机号登录、自动绑定本机和持续上报。
...
- Android 采集端编译验证：登录页、工作区、定位采集模块、自动设备绑定、设备凭证保存和前台服务后台上报代码路径已接入。
- Android 真机验证：手机号登录、自动绑定、本地后端联通和定位上报链路已跑通。
